...
class ListsSuite extends FunSuite {
   ...
   * sum
   */
  test("sum: few numbers") {
    assert(sum(List(1, 2, 0)) === 3)
  }
  
  test("sum: few negative numbers") {
    assert(sum(List(-11, -2, -10, -100)) === -123)
  }
   ...
   * max
   */
  test("max: few numbers") {
	  assert(max(List(3, 7, 2)) === 7)
  }
  
  test("max: few negative numbers") {
    assert(max(List(-11, -2, -10, -100)) === -2)
  }
  
  test("max: few negative and positive numbers") {
    assert(max(List(-11, -2, 10, -100, 13, 0)) === 13)
  }
  
  test("max: few numbers with repeated elements") {
    assert(max(List(13, -22 ,1 ,54 ,7, 54, 1)) === 54)
  }
